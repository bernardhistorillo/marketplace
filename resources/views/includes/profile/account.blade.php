...
<a class="text-color-6 text-decoration-none" href="{{ route('profile.index', $account) }}" id="profile-link" data-discount-signature="{{ $discountSignature['signature'] }}" data-own-token-balance="{{ $discountSignature['ownTokenBalance'] }}">
    <div class="d-flex align-items-center">
        <div class="background-image-cover bg-color-18 profile-photo" style="background-image:url('{{ ($user && $user['photo']) ? $user['photo'] : "https://avatars.dicebear.com/api/bottts/" . $account . ".svg?scale=80" }}'); width:35px; height:35px; border-radius:50%; border:1px solid #aaaaaa"></div>
